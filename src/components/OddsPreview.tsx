...
export default function OddsPreview() {
  const primary = BOXERS[0];
  const { opponent, oddsPrimary, oddsOpponent } = useMemo(() => proposeMatch(primary, BOXERS), [primary]);

  const [stake, setStake] = useState<number>(200);
  const [pick, setPick] = useState<"primary" | "opponent">("primary");
  const odds = pick === "primary" ? oddsPrimary : oddsOpponent;
        ...
        <div className="mt-10 grid lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl border border-white/10 bg-card overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-[1fr_auto_1fr] items-center gap-4">
              <Side boxer={primary} odds={oddsPrimary} active={pick === "primary"} onPick={() => setPick("primary")} />
              <div className="text-center text-muted font-mono text-sm">vs</div>
              <Side boxer={opponent} odds={oddsOpponent} active={pick === "opponent"} onPick={() => setPick("opponent")} />
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-card p-6">
            ...
            <div className="mt-3 flex items-center gap-3">
              <Avatar name={(pick === "primary" ? primary : opponent).name} hue={(pick === "primary" ? primary : opponent).avatarHue} size={40} />
              <div>
                <div className="text-sm font-semibold">{(pick === "primary" ? primary : opponent).name.split(" ").slice(0, -1).join(" ")}</div>
