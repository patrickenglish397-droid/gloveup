import { useAthletes } from "../lib/useApi";
import Avatar from "./Avatar";
import { SectionLabel } from "./HowItWorks";

export default function ProfilePreview() {
  const { data: boxers, loading, error } = useAthletes();
  const boxer = boxers?.[0] ?? null; // Mia
  ...
  return (
    <section id="profile" className="relative py-24 bg-gradient-to-b from-canvas to-card/30">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="mt-10 rounded-2xl border border-white/10 bg-card overflow-hidden min-h-[300px]">
          {loading && <div className="p-10 text-muted text-sm">Loading athlete…</div>}
          {error && <div className="p-10 text-rose-400 text-sm">Couldn't load: {error.message}</div>}
          {boxer && (() => {
            const totalFights = boxer.wins + boxer.losses + boxer.draws;
            const winRate = totalFights ? Math.round((boxer.wins / totalFights) * 100) : 0;
            return (
              <>
        ...
        <div className="mt-3 text-xs text-muted">
          Live data from gloveup-api · live app will pull from your linked Sport80 athlete record.
        </div>
      </div>
    </section>
  );
